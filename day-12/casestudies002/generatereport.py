...
avgs = sorted(set([class_dict[regid]['avg'] for regid in class_dict.keys()]), reverse=True)
rank_map = {avg: rank+1 for rank, avg in enumerate(avgs)}

for regid in class_dict.keys():
    class_dict[regid]['rank'] = rank_map[class_dict[regid]['avg']]
...
print(line)
with open('studrep.txt', 'w') as txt_file:
    template = "{0:8} | {1:15} | {2:5} | {3:5} | {4:5} | {5:5} | {6:5} | {7:5} | {8:5}"
    txt_file.write("\nCLASS REPORT\n")
    txt_file.write('-'*90 + '\n')
